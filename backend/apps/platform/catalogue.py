...
# services.get_enabled_modules). `admissions`, `academics` (via timetable),
# and `guardians` (bundled under student_records) have no api.py of their
# own yet -- included here for completeness/future-proofing even though
# there's no resolver to wire today. `tenancy` and `activity` are core
