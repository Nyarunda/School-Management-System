...
| `/finance/student-fee-assignments/` | GET/POST | `finance.student_account.view`/`finance.invoice.create` | `AssignmentsPage` — the "+ Assign fees" visibility gate was `finance.fee_structure.edit` (wrong permission, coupled assignment capability to fee-structure maintenance); fixed to `finance.invoice.create`, the permission `AssignmentListCreateView.create()`/`assign_fee_structure()` actually enforce. Verified live across all four YES/NO combinations of the two permissions. Student field was a raw exact-id `TextInput` (`STUDENT-GAP-02`); closed by wiring in `StudentSearchSelect` (the live server-side search built for `IncomingPage`'s Match dialog) after a user hit "No matching record" pasting an admission number where a UUID was expected | ✅ | ✅ |
| `/finance/fee-structures/<id>/bulk-assign/` | POST | `finance.invoice.create` | `FeeStructuresPage` — "Assign & invoice" on an approved structure, scoped to one class (via `/academics/class-groups/catalogue/`) or the whole level; assigns + invoices every actively enrolled `StudentEnrollment` match in one action, idempotent on re-run | ✅ | ✅ |
| `/finance/student-fee-assignments/<id>/generate-invoice/` | POST | `finance.invoice.create` | `AssignmentsPage` | ✅ | ✅ |
| `/finance/terms/<id>/generate-invoices/` | POST | `finance.invoice.create` | `AssignmentsPage` — "Generate invoices for term", a term-wide invoice-only catch-up sweep across every class/level, independent of how each assignment was created | ✅ | ✅ |
| `/finance/invoices/` | GET | `finance.invoice.view` | `InvoicesPage` | ✅ | ✅ — student filter now `StudentSearchSelect`, not the former raw-id field (`STUDENT-GAP-02`) |
...
| `/finance/fee-categories/` | GET/POST | `finance.setup.view`/`.manage` | `features/finance.tsx` `FeeItemsPage` | ✅ | ✅ — closed a real gap: the endpoint always existed but no frontend page could create a category, so `FeeStructuresPage`'s "Add line" fee-item picker was permanently empty on every real tenant |
| `/finance/fee-items/` | GET/POST | `finance.setup.view`/`.manage` | `FeeItemsPage` | ✅ | ✅ |
| `/finance/payments/` | GET/POST | `finance.payment.view`/`.record` | `PaymentsPage` | ✅ | ✅ — student filter and Record-payment picker both now `StudentSearchSelect` (the record dialog was previously a `page_size:100` dropdown, silently incomplete past 100 students; the filter was a raw-id field, `STUDENT-GAP-02`) |
