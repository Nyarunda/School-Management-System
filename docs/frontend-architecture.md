...
"State transition where the consequence matters" is guidance for *choosing* a confirmation `Modal` when a consequential action is being built or a real defect requires one — it is not license to retroactively add a confirmation step to an existing direct action (e.g. M-Pesa's "Process verified callback" button) during a presentation-only migration. If an action's confirmation semantics genuinely need to change, that's its own reviewable decision, not a side effect of moving a page onto Mantine.
