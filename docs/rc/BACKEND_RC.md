...
### Check 4 — Celery worker graceful restart mid-task ✅ DONE
...
**Resumed with a different method, per explicit direction: stop trying to force business logic into blocking; deterministically control a real task via external process control instead — no new production code, no test-only task added to application runtime.** Two earlier attempts (`select_for_update()` lock-holding on a `NotificationOutbox`/`IncomingPayment` row, documented above) both failed to produce a genuinely blocking real task: `claim_due()` uses `select_for_update(skip_locked=True)` by design, so it never blocks; `_attempt_auto_match` short-circuited before reaching its own lock. Neither is a defect — both are the correct, already-proven-safe behavior of that code (Area 5 Check 3's skip-locked concurrency design; `_attempt_auto_match`'s cheap early-exit when there's nothing to match) — they just made this specific check's job harder.

**Method (no code changes, no business-data engineering):** the real Celery prefork pool's own child processes, frozen and resumed via bare OS signals from a driver in the `celery-worker` container itself:
1. Found the pool's real container-internal PIDs (`/proc` scan, matching Area 5 Check 2's own established method): MainProcess `1`, four `ForkPoolWorker` children `11`/`12`/`13`/`14` (`--concurrency=4`).
2. Created a genuine `NotificationOutbox` row via a real invite (same pattern as Area 5).
3. `os.kill(pid, signal.SIGSTOP)` on all four children **before** dispatching anything — confirmed via `/proc/<pid>/status` showing `State: T (stopped)` for all four, a real, deterministic freeze, not a timing guess.
4. Dispatched the actual registered `apps.notifications.tasks.dispatch_pending_notifications` via `.delay()` (the real function Celery invokes, not a reimplementation) — confirmed `received` logged, no `succeeded`, genuinely stuck: a message handed to a `SIGSTOP`'d OS process can't be acted on at all, so `claim_due()` inside it never even ran (row stayed `PENDING`, `attempts=0` throughout — not orphaned mid-claim, simply never reached).
5. `docker compose restart celery-worker` (real SIGTERM to MainProcess, PID 1 directly — this container's command has no shell-wrapping issue, unlike the now-fixed loadtest-overlay defect above). Celery's own log immediately showed **`worker: Warm shutdown (MainProcess)`** — direct confirmation the signal was received and acted on correctly.
6. Attempted to `SIGCONT` the frozen children ~3.8s later, still expecting to be within a graceful grace period — but the container had **already been torn down** (`service "celery-worker" is not running`) by then.

**Finding, not a defect — graceful shutdown's protection is bounded, not indefinite:** Celery correctly receives and acts on SIGTERM (the "Warm shutdown" log proves that), but it does not wait indefinitely for a task that isn't making progress — this configuration (no `worker_shutdown_timeout` set, no custom `stop_grace_period` in compose) tore the container down within a few seconds regardless of the still-in-flight task. **The practical implication**: don't rely on "graceful restart" itself to protect in-flight Celery work — a stuck or slow-enough task can still be interrupted by a restart. What actually keeps the system correct is the same thing Area 5 already proved: idempotent claim/lease + the standing reap/redispatch cycle. This run is direct evidence of exactly that, not a new mechanism: the interrupted dispatch left the row cleanly `PENDING` (never claimed, so nothing to reclaim), and the ordinary, unmodified dispatch cycle picked it up once `celery-beat`/`celery-worker` were both healthy again — final state **`PROCESSED`, `attempts=1`, exactly one `NotificationDeliveryAttempt` (`SENT`/`EMAIL`)**. No duplication, no stuck state, no manual intervention, despite a genuinely adversarial interruption (not just a crash — an interruption engineered specifically to prevent the warm-shutdown path from ever completing).

**Classification:** **PASS**, no defect. Graceful SIGTERM handling works correctly (confirmed live, not just by code reading); its protection is bounded rather than indefinite (a real, worth-remembering operational fact, not a bug); and correctness under interruption — the actual invariant this area cares about — holds regardless, because it rests on Area 5's claim/lease design, not on warm shutdown succeeding. No code change made.

### Check 5 — Celery Beat restart, schedule integrity ✅ DONE

**Claim under test:** the default `PersistentScheduler` persists its own last-run bookkeeping to a local file (`celerybeat-schedule`, a Python `shelve`/dbm file) rather than the database. This dev compose bind-mounts `./backend:/app`, so that file happens to land on the host and survive a container restart/recreate — an accident of the dev bind mount, not a deliberate design choice for the file itself. A real no-bind-mount production deploy would NOT have that file at all on a fresh container. Per the user's explicit refinement: don't require literal exactly-once scheduling (periodic schedulers are safely at-least-once by design, idempotency lives at the task boundary); the bar is no harmful duplicate side effects, no permanently lost required work, eventual resumption — and if the schedule file disappearing changes behavior, document that plainly rather than disguising it.

**Method, two scenarios, both against the live stack:**
- **Test A — schedule file intact** (this environment's actual behavior, `docker compose restart celery-beat`): established a baseline first — the two hourly tasks (`finance-resweep-unmatched-incoming-payments`, `documents-purge-expired`, both 3600s interval) had last genuinely fired at `18:42:36`, not due again until `~19:42:36`. Restarted Beat at `19:27:19`. **Result: neither hourly task re-fired** — only tasks actually due at that moment fired, exactly matching what a correctly-loaded persisted schedule should produce.
- **Test B — schedule file absent**, simulating a real no-bind-mount redeploy: stopped Beat, backed up (not deleted — `cp` then `rm`, restored afterward) `/app/celerybeat-schedule`, started Beat completely fresh with zero prior history. **Result: no crash, no error, no immediate mass re-fire of every periodic task.** The 30s-interval tasks fired almost immediately (expected — anything with a short-enough period is essentially always "due" regardless of history), but the two hourly tasks still had **not** fired 2.5 minutes after the fresh boot. This confirms `PersistentScheduler`'s actual behavior on missing state: an entry with no `last_run_at` gets initialized relative to the scheduler's own boot time, not treated as infinitely overdue — each task's cadence simply resets to count from the new boot forward, it does not trigger a startup storm.
