...
export function FeeStructuresPage(){
	...
	const approve=useMutation({mutationFn:(id:string)=>api(`/finance/fee-structures/${id}/approve/`,{method:"POST"}),onSuccess:()=>{void qc.invalidateQueries({queryKey:["fee-structures"]});setSelected(null)}});
	...
	const years=useQuery({queryKey:["academic-years"],queryFn:()=>api<Page<AcademicYear>>("/academics/academic-years/",{params:{page_size:100}}),enabled:canCreate});
	const levels=useQuery({queryKey:["academic-levels"],queryFn:()=>api<Page<AcademicLevel>>("/academics/academic-levels/",{params:{page_size:100}}),enabled:canCreate});
	const create=useMutation({mutationFn:()=>api<Structure>("/finance/fee-structures/",{method:"POST",body:JSON.stringify({name,academic_year:year,academic_level:level})}),onSuccess:()=>{void qc.invalidateQueries({queryKey:["fee-structures"]});setCreateOpen(false);setName("");setYear("");setLevel("")}});
	...
	const addLine=useMutation({
		...
		onSuccess:newLine=>{setSelected(prev=>prev?{...prev,lines:[...prev.lines,newLine]}:prev);setLineItem("");setLineAmount("");void qc.invalidateQueries({queryKey:["fee-structures"]})},
	});

	const columns:Column<Structure>[]=[{key:"name",header:"Structure",cell:r=><><strong>{r.name}</strong><small className="cell-sub">{r.lines.length} fee lines</small></>},{key:"year",header:"Academic year",cell:r=><code>{r.academic_year}</code>},{key:"level",header:"Level",cell:r=><code>{r.academic_level}</code>},{key:"total",header:"Total",cell:r=>cash(r.lines.reduce((n,l)=>n+Number(l.amount),0))},{key:"status",header:"Status",cell:r=><StatusBadge value={r.is_approved?"Approved":"Draft"}/>}];
...
export function PaymentsPage(){
	...
	const mutate=useMutation({mutationFn:()=>api(mode==="allocate"?`/finance/payments/${selected!.id}/allocate/`:`/finance/payments/${selected!.id}/reverse/`,{method:"POST",body:JSON.stringify(mode==="allocate"?{invoice,amount}:{reason})}),onSuccess:()=>{void qc.invalidateQueries({queryKey:["payments"]});setMode(null);setSelected(null)}});
	...
	const recordStudents=useQuery({queryKey:["payment-record-students"],queryFn:()=>api<Page<Student>>("/students/",{params:{page_size:100}}),enabled:canRecord});
	...
	return <>
		...
		<ActionDialog open={recordOpen} title="Record payment" description="Creates a received payment for a student, ready to allocate against an invoice." confirmLabel="Record payment" busy={record.isPending} onClose={()=>setRecordOpen(false)} onSubmit={e=>{e.preventDefault();record.mutate()}}>
			...
			<label>Student<select required value={payStudent} onChange={e=>setPayStudent(e.target.value)}><option value="">Select student</option>{recordStudents.data?.results.map(s=><option key={s.id} value={s.id}>{s.full_name} · {s.admission_number}</option>)}</select></label>
